...
static void fed_drain_cb(const V4xCommitEntry *e, void *ud)
{
    ...
    /* build packed cell per fed_gate layout:
     *   hil  = bit[19:0]  → e->lane (so hil%54 == lane ✓)
     *   lane = bit[25:20] → e->lane
     *   iso  = bit[26]    → 0 (normal write)               */
    uint8_t  lane   = e->lane;                          /* v_clean % 54 */
    uint32_t hil    = (uint32_t)lane;                   /* hil%54==lane  */
    uint32_t packed = (hil & 0xFFFFFu)
                    | ((uint32_t)(lane & 0x3Fu) << 20); /* iso=0         */
    fed_write(fed,
              packed,
                                 ...
                                 uint32_t v_raw)
{
    ...
    /* op_count sync: tie federation warm-up to V4x total_steps
     * so fed_gate warm-up (< GHOST_STREAK_MAX) clears early      */
    fed->op_count = w->total_steps;
