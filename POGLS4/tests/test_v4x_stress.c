...
static void test_over_correction(void)
{
     ...
     * and compare against anchor_changes. Healthy = 5..80% of anchor fires.  */
    {
        ...
        /* anchor_changes counts per-core (N cores updated per event),
         * so normalise by N to get per-event change rate.               */
        uint32_t N = (w.ma.N > 0) ? w.ma.N : 1;
        double rate = (double)w.ma.anchor_changes / ((double)total_anchors * N);
