...
<main>
  ...
  <section class="row two">
    ...
    <article class="card">
      ...
      <div class="table-wrap" style="max-height:260px"><table><thead><tr><th>Repo</th><th>Archive</th><th>Version</th><th>Excluded</th></tr></thead><tbody id="archivesBody"></tbody></table></div>
    </article>
  </section>
</main>
<script>
...
function renderArchives(){ const rows=[]; Object.entries(SNAP.archives||{}).forEach(([repo, items])=>items.slice(0,60).forEach(a=>rows.push(`<tr><td>${repo}</td><td>${a.path}</td><td>${a.version||'-'}</td><td>${a.excluded?'yes':'no'}</td></tr>`))); document.getElementById('archivesBody').innerHTML=rows.join('') || `<tr><td colspan="4" class="muted">No archives inventoried.</td></tr>`; }
